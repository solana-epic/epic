...
impl<'a> FileVisitor<'a> {
    fn insert_def(&mut self, name: String, def: TypeDef) {
        ...
        self.registry
            .module_paths
            .insert(abs_path.clone(), self.current_module_path.clone());
    ...
    fn insert_instruction_def(&mut self, mod_name: &str, name: String, def: TypeDef) {
        ...
        self.registry
            .module_paths
            .insert(abs_path.clone(), mod_path);
        self.registry.symbol_names.insert(abs_path.clone(), name);
        if let Some(ref fp) = self.file_path {
            self.registry.file_paths.insert(abs_path, fp.clone());
        }
        ...
        if let syn::Fields::Named(named) = &i.fields {
            for field in &named.named {
                if let Some(ident) = &field.ident {
                    ...
                    // Capture the source span of this field declaration using proc_macro2.
                    // span().start() gives a 1-indexed line and 0-indexed column.
                    let span_start = ident.span().start();
                    fields.push(FieldDef {
                        name: ident.to_string(),
                        type_ref,
                        attrs,
                        line_number: span_start.line,
                        column_number: span_start.column,
                    });
                }
            }
        }
