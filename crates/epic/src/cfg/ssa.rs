...
impl<'a> SSAComputer<'a> {
    ...
    fn propagate_node(
        ...
        if let Some(node) = self.graph.nodes.get(&u) {
            ...
            // For now we just process IR Instructions separately in a throwaway state
            // to verify compilation without breaking existing test behaviour.
            let mut throwaway_state = current_state.clone();
            for instr in &node.ir_instructions {
                let mut declared = Vec::new();
                self.propagate_ir_instruction(instr, &mut throwaway_state, &mut declared);
            }
        }
    ...
    fn propagate_ir_instruction(
        &mut self,
        instr: &epic_ir::IRInstruction,
        current_state: &mut SSANodeState,
        _declared_in_block: &mut [String],
    ) {
        if let epic_ir::IRInstruction::Assignment(assignment) = instr {
            let next_ver = self
                .version_counters
            ...
            *next_ver += 1;
            ...
            if let InferenceResult::Ok(type_ref) = engine.infer_ir(&assignment.value) {
                current_state
                    .variable_types
            ...
            current_state
                .active_variables
                .insert(assignment.target.clone(), ssa_var);
        }
    }
